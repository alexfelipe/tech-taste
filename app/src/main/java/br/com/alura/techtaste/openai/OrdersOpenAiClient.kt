...
class OrdersOpenAiClient(
    private val openAI: OpenAI
) {

    suspend fun getMessageAndOrders(text: String): Pair<String, List<Order>>? {
        val openAiResponse = chatCompletion(text)
            .choices
            .mapNotNull {
                it.message.content
            }.joinToString(separator = "")
        return convertToMessageAndOrders(openAiResponse)
    }

    private fun convertToMessageAndOrders(openAiResponse: String): Pair<String, List<Order>>? {
        val jsonPattern = "\\{.*\\}".toRegex(RegexOption.DOT_MATCHES_ALL)
        val jsonMatch = jsonPattern.find(openAiResponse)
        return jsonMatch?.value?.let { rawJson ->
